/**
 * Phase 41.17.2 — Manual Library Content-Health Helper Runner (pure logic)
 *
 * The deterministic, testable core of the manual CLI runner for the v1
 * `library_content_health_helper`. PURE: no I/O, no DB, no Supabase, no fetch, no
 * Anthropic/OpenAI. The CLI (`scripts/run-library-content-health-helper.ts`)
...
export const LIBRARY_CONTENT_HEALTH_HELPER_VERSION = '41.17.2'
...
export function dedupeKeyForDraft(
  ...
  helperVersion: string = LIBRARY_CONTENT_HEALTH_HELPER_VERSION,
): string {

/**
 * Phase 41.17.2 — Manual Source Reference Integrity Helper Runner (pure logic)
 *
 * The deterministic, testable core of the manual CLI runner for the v1
 * `source_reference_integrity_helper`. PURE: no I/O, no DB, no Supabase, no fetch,
 * no Anthropic/OpenAI. The CLI
 * (`scripts/run-source-reference-integrity-helper.ts`) supplies all I/O and calls
 * into here for argument validation, dedupe-key derivation, metadata stamping, and
 * the deposit plan.
 *
...
export const SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION = '41.17.2'
...
export function dedupeKeyForDraft(
  ...
  helperVersion: string = SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION,
): string {

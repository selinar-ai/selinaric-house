/**
 * Phase 42.1 — Manual Helper Runner (pure logic)
 *
 * The deterministic, testable core of the manual CLI runner for the v1
 * `library_metadata_helper`. PURE: no I/O, no DB, no Supabase, no fetch, no
 * Anthropic/OpenAI. The CLI (`scripts/run-library-metadata-helper.ts`) supplies
 * all I/O and calls into here for argument validation, dedupe-key derivation,
 * metadata stamping, and the deposit plan.
 *
 * ── Law ──────────────────────────────────────────────────────────────────────
 ...
 *
 * Dedupe rule (Ari, tightened): a candidate is skipped when an ACTIVE (non-soft-
 * deleted) helper_outputs row already exists with the same `_dedupe_key` — across
 * ALL review states (unreviewed / viewed / needs_action / needs_decision /
 * dismissed). Soft-deleted rows do NOT block. A helper-version change yields a
 * new key. v1 never recreates dismissed/viewed work automatically.
 */
...
export const LIBRARY_METADATA_HELPER_VERSION = '42.1.0'

export const RUNNER_INVOKED_BY = 'manual_cli' as const

export type RunMode = 'test_owned' | 'real_deposit'
...
export type RunnerArgs =
  | { ok: true; libraryItemId: string; depositReal: boolean; runMode: RunMode }
 ...
 * comma-separated ids. Real deposits require an explicit `--deposit-real`;
 * otherwise the run is test-owned.
 */
export function parseRunnerArgs(argv: readonly string[]): RunnerArgs {
  ...
  return { ok: true, libraryItemId: id, depositReal, runMode: depositReal ? 'real_deposit' : 'test_owned' }
}
...
export function dedupeKeyForDraft(
  ...
  helperVersion: string = LIBRARY_METADATA_HELPER_VERSION,
): string {

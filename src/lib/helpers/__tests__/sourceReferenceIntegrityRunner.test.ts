/**
 * Phase 41.17.2 — Source Reference Integrity Runner tests
 *
 * Run: npx tsx src/lib/helpers/__tests__/sourceReferenceIntegrityRunner.test.ts
 *
...
import { inspectSourceReferenceIntegrity, type SourceRefItemSnapshot, type SourceRefFileSnapshot } from '../sourceReferenceIntegrityHelper'
...
import {
  ...
  stampedDedupeKey,
  planDeposit,
  SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION,
  RUNNER_INVOKED_BY,
} from '../sourceReferenceIntegrityRunner'
...
// An item that trips all three checks → 3 drafts.
const FIXTURE: SourceRefItemSnapshot = {
  id: 'item-1', presence_scope: 'house',
  source_url: 'not-a-url', file_path: 'claimed/path.pdf',
}
const FILES: SourceRefFileSnapshot[] = [
  { id: 'broken-1', library_item_id: 'item-1', file_path: null, storage_bucket: 'library-files' },
]
const DRAFTS = inspectSourceReferenceIntegrity(FIXTURE, FILES)
...
{
  const k = computeDedupeKey('source_reference_integrity_helper', '41.17.2', 'item-1', 'source_url_malformed')
  assert(/^[a-f0-9]{64}$/.test(k), 'dedupe key is a 64-char sha256 hex')
  assert(computeDedupeKey('source_reference_integrity_helper', '41.17.2', 'item-1', 'source_url_malformed') === k, 'same inputs → same key (deterministic)')
  assert(computeDedupeKey('source_reference_integrity_helper', '99.0.0', 'item-1', 'source_url_malformed') !== k, 'helper-version change → new key')
  assert(computeDedupeKey('source_reference_integrity_helper', '41.17.2', 'item-2', 'source_url_malformed') !== k, 'different item → different key')
  assert(computeDedupeKey('source_reference_integrity_helper', '41.17.2', 'item-1', 'file_storage_reference_broken') !== k, 'different issue → different key')
...
section('C. Metadata stamping')
{
  ...
  const stamped = stampRunnerMetadata(d, { helperVersion: SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION, dedupeKey: key, runId: 'run-x', runMode: 'test_owned' })
  const p = stamped.suggestion_payload as Record<string, unknown>
  assert(p.helper_version === SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION, 'stamps helper_version')
  ...
  assert(typeof p.issue_code === 'string' && p.issue_code === (d.suggestion_payload as { issue_code: string }).issue_code, 'preserves the original issue payload')
  assert(stampedDedupeKey(stamped) === key, 'stampedDedupeKey reads the key back')
  assert(stamped.helper_type === 'source_reference_integrity_helper', 'helper_type unchanged')
...
{
  const stamped = DRAFTS.map((d) => stampRunnerMetadata(d, { helperVersion: SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId: 'r', runMode: 'test_owned' }))
...
{
  const d = DRAFTS[0]
  const stamped = stampRunnerMetadata(d, { helperVersion: SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId: 'r', runMode: 'test_owned' })
  ...
  assert(test.helper_type === 'source_reference_integrity_helper', 'helper type is the source-reference helper (not widened)')
...
{
  const cli = readSrc('../../../../scripts/run-source-reference-integrity-helper.ts')
  ...
  assert(cli.includes('library_item_files?library_item_id=eq.'), 'reads that item\'s files (for storage metadata)')
  ...
  assert(cli.includes('inspectSourceReferenceIntegrity') && cli.includes('insertHelperOutputs'), 'reuses the sealed helper + writer unchanged')
}

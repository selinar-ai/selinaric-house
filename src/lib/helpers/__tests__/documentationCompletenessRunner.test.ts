/**
 * Phase 41.17.2 — Documentation Completeness Runner tests
 *
 * Run: npx tsx src/lib/helpers/__tests__/documentationCompletenessRunner.test.ts
 *
 ...
 *  F. CLI static scan — reads only the Library item surface; INSERT only; dry-run
 *     writes nothing; no files read; no route / candidate read / update / delete / scheduler
 */

import * as fs from 'fs'
import * as path from 'path'

import { inspectDocumentationCompleteness, type DocCompletenessItemSnapshot } from '../documentationCompletenessHelper'
...
import {
  ...
  RUNNER_INVOKED_BY,
} from '../documentationCompletenessRunner'
...
const FIXTURE: DocCompletenessItemSnapshot = {
  id: 'item-1', presence_scope: 'house', collection: 'development_documentation',
  phase_code: 'P1', phase_number: null, phase_label: null,
  authority_status: 'superseded', archive_item_id: null,
}
const DRAFTS = inspectDocumentationCompleteness(FIXTURE)
...
{
  const k = computeDedupeKey('documentation_completeness_helper', '41.17.2', 'item-1', 'phase_doc_incomplete_phase_metadata')
  assert(/^[a-f0-9]{64}$/.test(k), 'dedupe key is a 64-char sha256 hex')
  assert(computeDedupeKey('documentation_completeness_helper', '41.17.2', 'item-1', 'phase_doc_incomplete_phase_metadata') === k, 'same inputs → same key (deterministic)')
  assert(computeDedupeKey('documentation_completeness_helper', '41.17.3', 'item-1', 'phase_doc_incomplete_phase_metadata') !== k, 'helper-version change → new key')
  assert(computeDedupeKey('documentation_completeness_helper', '41.17.2', 'item-2', 'phase_doc_incomplete_phase_metadata') !== k, 'different item → different key')
  assert(computeDedupeKey('documentation_completeness_helper', '41.17.2', 'item-1', 'superseded_item_missing_archive_link') !== k, 'different issue → different key')
...
section('C. Metadata stamping')
{
  ...
  const stamped = stampRunnerMetadata(d, { helperVersion: DOCUMENTATION_COMPLETENESS_HELPER_VERSION, dedupeKey: key, runId: 'run-x', runMode: 'test_owned' })
  const p = stamped.suggestion_payload as Record<string, unknown>
  assert(p.helper_version === DOCUMENTATION_COMPLETENESS_HELPER_VERSION, 'stamps helper_version')
  ...
  assert(p.issue_code === 'phase_doc_incomplete_phase_metadata', 'preserves the original issue payload')
  assert(stampedDedupeKey(stamped) === key, 'stampedDedupeKey reads the key back')
  assert(stamped.helper_type === 'documentation_completeness_helper', 'helper_type unchanged')
...
{
  const stamped = DRAFTS.map((d) => stampRunnerMetadata(d, { helperVersion: DOCUMENTATION_COMPLETENESS_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId: 'r', runMode: 'test_owned' }))
...
{
  const d = DRAFTS[0]
  const stamped = stampRunnerMetadata(d, { helperVersion: DOCUMENTATION_COMPLETENESS_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId: 'r', runMode: 'test_owned' })
  ...
  assert(test.helper_type === 'documentation_completeness_helper', 'helper type is the documentation-completeness helper (not widened)')
...
{
  const cli = readSrc('../../../../scripts/run-documentation-completeness-helper.ts')
  ...
  // Reads ONLY the Library item surface; one named item; NO files.
  assert(cli.includes('library_items?id=eq.'), 'reads one library item by id (no scan, no all)')
  assert(!cli.includes('library_item_files'), 'this helper never reads files (no library_item_files)')
  ...
  assert(cli.includes('inspectDocumentationCompleteness') && cli.includes('insertHelperOutputs'), 'reuses the sealed helper + writer unchanged')
}

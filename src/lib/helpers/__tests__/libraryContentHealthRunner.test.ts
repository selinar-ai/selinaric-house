/**
 * Phase 41.17.2 — Library Content-Health Runner tests
 *
 * Run: npx tsx src/lib/helpers/__tests__/libraryContentHealthRunner.test.ts
 *
...
import {
  ...
  RUNNER_INVOKED_BY,
} from '../libraryContentHealthRunner'
...
// An item with one truncated file AND one needs_review file → 2 drafts.
const FIXTURE_ITEM: ContentHealthItemSnapshot = { id: 'item-1', presence_scope: 'house' }
const FIXTURE_FILES: ContentHealthFileSnapshot[] = [
  { id: 'file-trunc', library_item_id: 'item-1', extraction_status: 'extracted', extraction_truncated: true, needs_review: false },
  { id: 'file-flag', library_item_id: 'item-1', extraction_status: 'extracted', extraction_truncated: false, needs_review: true },
]
const DRAFTS = inspectLibraryContentHealth(FIXTURE_ITEM, FIXTURE_FILES)
...
{
  const k = computeDedupeKey('library_content_health_helper', '41.17.2', 'item-1', 'file_content_truncated')
  assert(/^[a-f0-9]{64}$/.test(k), 'dedupe key is a 64-char sha256 hex')
  assert(computeDedupeKey('library_content_health_helper', '41.17.2', 'item-1', 'file_content_truncated') === k, 'same inputs → same key (deterministic)')
  assert(computeDedupeKey('library_content_health_helper', '41.17.3', 'item-1', 'file_content_truncated') !== k, 'helper-version change → new key')
  assert(computeDedupeKey('library_content_health_helper', '41.17.2', 'item-2', 'file_content_truncated') !== k, 'different item → different key')
  assert(computeDedupeKey('library_content_health_helper', '41.17.2', 'item-1', 'file_flagged_needs_review') !== k, 'different issue → different key')
...
section('C. Metadata stamping')
{
  ...
  const stamped = stampRunnerMetadata(d, { helperVersion: LIBRARY_CONTENT_HEALTH_HELPER_VERSION, dedupeKey: key, runId: 'run-x', runMode: 'test_owned' })
  const p = stamped.suggestion_payload as Record<string, unknown>
  assert(p.helper_version === LIBRARY_CONTENT_HEALTH_HELPER_VERSION, 'stamps helper_version')
  ...
  assert(p.issue_code === code, 'preserves the original issue payload')
  assert(stampedDedupeKey(stamped) === key, 'stampedDedupeKey reads the key back')
  assert(stamped.helper_type === 'library_content_health_helper', 'helper_type unchanged')
...
{
  const stamped = DRAFTS.map((d) => stampRunnerMetadata(d, { helperVersion: LIBRARY_CONTENT_HEALTH_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId: 'r', runMode: 'test_owned' }))
...
{
  const d = DRAFTS[0]
  const stamped = stampRunnerMetadata(d, { helperVersion: LIBRARY_CONTENT_HEALTH_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId: 'r', runMode: 'test_owned' })
  ...
  assert(test.helper_type === 'library_content_health_helper', 'helper type is the content-health helper (not widened)')
...
{
  const cli = readSrc('../../../../scripts/run-library-content-health-helper.ts')
  ...
  assert(cli.includes('library_item_files?library_item_id=eq.'), 'reads that item\'s files (metadata only)')
  ...
  assert(cli.includes('inspectLibraryContentHealth') && cli.includes('insertHelperOutputs'), 'reuses the sealed helper + writer unchanged')
}

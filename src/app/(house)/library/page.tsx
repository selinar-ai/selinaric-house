...
// Phase 33B + 33C + 33F — House Library v1 + File Attachments + Retrieval Lab
//
...
// Phase 33F: Retrieval Lab — deterministic retrieval preview, no chat injection
//
...
import {
  COLLECTIONS, ITEM_TYPES, AUTHORITY_LABELS, AUTHORITY_COLORS, PRESENCE_LABELS,
  EXTRACTION_METHOD_LABELS,
} from '@/lib/library/constants'
import type { Collection, ItemType, LibraryItem, LibraryFile } from '@/lib/library/constants'
...
// Types and constants imported from @/lib/library/constants
...
export default function LibraryPage() {
  // View toggle: Library or Retrieval Lab
  const [activeView, setActiveView] = useState<'library' | 'retrieval-lab'>('library')
  ...
  return (
    <div className="flex flex-col min-h-full">
          ...
          <div>
            ...
            <p className="font-body text-xs text-text-muted mt-0.5">
              {activeView === 'library'
                ...
                : 'Preview Library context before it is used anywhere else.'
              }
            </p>
          </div>
          <div className="flex items-center gap-2">
            {/* View toggle */}
            <div className="flex border border-house-border">
              <button
                onClick={() => setActiveView('library')}
                className={`font-body text-[10px] px-2.5 py-1 transition-all ${
                  activeView === 'library'
                    ? 'bg-house-bg text-text-secondary'
              ...
              <button
                onClick={() => setActiveView('retrieval-lab')}
                className={`font-body text-[10px] px-2.5 py-1 border-l border-house-border transition-all ${
                  activeView === 'retrieval-lab'
                    ? 'bg-house-bg text-text-secondary'
                    : 'text-text-muted hover:text-text-secondary'
                }`}
              >
      ...
      {activeView === 'library' && <div className="flex flex-1 min-h-0">
      ...
      {activeView === 'library' && formWarning && !formOpen && (
      ...
      {activeView === 'library' && uploadErrors.length > 0 && !formOpen && (

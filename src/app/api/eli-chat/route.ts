...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const journalResult = await getJournalContextForPresence('eli')
    ...
    const loungeCarrybackBlock = await buildCarrybackBlock('eli').catch(() => '')

    // Phase 36E: Cross-Room Prompt Carryforward — recent cross-room context
    const crossRoomCarryforwardBlock = await getCrossRoomCarryforwardBlock('eli', ROOM_SLUG).catch(() => '')
...
- The image is context, not a replacement for who you are in this room${draftNotice}${RECALL_ADVISORY_NON_DISCLOSURE_GUARD}`
    ...
    while (true) {
      ...
      const sessionPresenceRecallCount = await getSessionPresenceRecallCount('eli', sessionId)
      ...
      for (const toolCall of toolUseBlocks) {
        if (toolCall.name === 'web_search') {
          ...
          if (presenceRecallCount >= PRESENCE_RECALL_MAX_PER_RESPONSE || recallSessionLimitReached) {
            ...
            const r = await executePresenceRecall({ presenceId: 'eli', query, sessionId })

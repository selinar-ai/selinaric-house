/**
 * Phase 41.17.2 — Manual Helper Runner for the Source Reference Integrity Agent (CLI, MANUAL ONLY)
 *
 * Runs the deterministic v1 `source_reference_integrity_helper` against ONE named
 ...
 *     npx tsx scripts/run-source-reference-integrity-helper.ts --confirm-helper-run --library-item-id <id> --dry-run
 *
 *   Test-owned run (default — safe, cleanable):
 *     npx tsx scripts/run-source-reference-integrity-helper.ts --confirm-helper-run --library-item-id <id>
 *
 *   Real deposit (separate approval required — writes non-test rows that persist):
 *     npx tsx scripts/run-source-reference-integrity-helper.ts --confirm-helper-run --library-item-id <id> --deposit-real
 *
...
} from '../src/lib/helpers/sourceReferenceIntegrityHelper'
import {
  insertHelperOutputs,
...
} from '../src/lib/helpers/sourceReferenceIntegrityRunner'
...
function refuse(message: string): never {
  console.error(`\n[run-source-reference-integrity-helper] REFUSED: ${message}\n`)
  console.error('Usage:')
...
async function main() {
  ...
  // Read ONE library item (+ its file metadata). Read-only; never a scan.
  const itemRows = await restGet<{
    id: string; presence_scope: string | null
    source_url: string | null; file_path: string | null
  }>(rest, `library_items?id=eq.${encodeURIComponent(libraryItemId)}&select=id,presence_scope,source_url,file_path`)
  const itemRow = itemRows[0]
  if (!itemRow) refuse(`Library item not found: ${libraryItemId}`)

  const item: SourceRefItemSnapshot = {
    id: itemRow.id,
  ...
  const fileRows = await restGet<{ id: string; library_item_id: string; file_path: string | null; storage_bucket: string | null }>(
    rest,
    `library_item_files?library_item_id=eq.${encodeURIComponent(libraryItemId)}&select=id,library_item_id,file_path,storage_bucket`,
  )
  const files: SourceRefFileSnapshot[] = fileRows.map((f) => ({
    id: f.id,
  ...
  console.log(`[run-source-reference-integrity-helper] item ${libraryItemId} · ${files.length} file(s) · mode=${runMode} · helper v${SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION}`)

  // Deterministic inspection (clean item → nothing). Then stamp each draft.
  const drafts = inspectSourceReferenceIntegrity(item, files)
  if (drafts.length === 0) {
    console.log('[run-source-reference-integrity-helper] No source-reference integrity issues found — nothing to deposit.')
    process.exit(0)
  }
  const stamped = drafts.map((d) =>
    stampRunnerMetadata(d, { helperVersion: SOURCE_REFERENCE_INTEGRITY_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId, runMode }),
  )

  // Dry run: preview the candidates and write NOTHING (no dedupe query, no insert).
  if (dryRun) {
    console.log(`\n[run-source-reference-integrity-helper] DRY RUN — ${stamped.length} candidate(s), nothing written:`)
  ...
  if (plan.toInsert.length === 0) {
    console.log(`[run-source-reference-integrity-helper] All ${stamped.length} candidate(s) already active (dedupe) — nothing new deposited.`)
    process.exit(0)
  }

  const inserted = await insertHelperOutputs(restHelperOutputClient(rest), plan.toInsert, { testOwned: !depositReal })

  console.log(`\n[run-source-reference-integrity-helper] Deposited ${inserted.length} row(s) · skipped ${plan.skipped.length} (dedupe) · run_id=${runId} · test_owned=${!depositReal}`)
...
main().catch((e) => { console.error(`\n[run-source-reference-integrity-helper] ERROR: ${e?.message ?? e}\n`); process.exit(1) })

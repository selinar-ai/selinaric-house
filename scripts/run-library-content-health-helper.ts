/**
 * Phase 41.17.2 — Manual Helper Runner for the Library Content-Health Agent (CLI, MANUAL ONLY)
 *
 * Runs the deterministic v1 `library_content_health_helper` against ONE named
 ...
 *     npx tsx scripts/run-library-content-health-helper.ts --confirm-helper-run --library-item-id <id> --dry-run
 *
 *   Test-owned run (default — safe, cleanable):
 *     npx tsx scripts/run-library-content-health-helper.ts --confirm-helper-run --library-item-id <id>
 *
 *   Real deposit (separate approval required — writes non-test rows that persist):
 *     npx tsx scripts/run-library-content-health-helper.ts --confirm-helper-run --library-item-id <id> --deposit-real
 *
 * Boundaries: one named item per run (no "all"); refuses without an explicit
 * confirmation flag; reads only `library_items` / `library_item_files` (file
 * METADATA only — never extracted_text); INSERT only (never updates/deletes/
 * upserts); never sets review/authority/deleted fields (the writer forbids them);
 * never reads candidate / Memory / Graph surfaces; no route, no migration, no
 * schema change. Never wired to cron, chat, prompts, or the UI — manual
 * invocation only.
 */
...
} from '../src/lib/helpers/libraryContentHealthHelper'
import {
  insertHelperOutputs,
...
import {
  ...
  stampedDedupeKey,
  planDeposit,
  LIBRARY_CONTENT_HEALTH_HELPER_VERSION,
} from '../src/lib/helpers/libraryContentHealthRunner'
...
function refuse(message: string): never {
  console.error(`\n[run-library-content-health-helper] REFUSED: ${message}\n`)
  console.error('Usage:')
...
async function main() {
  ...
  // Read ONE library item (+ its files' metadata). Read-only; never a scan.
  const itemRows = await restGet<{
    id: string; presence_scope: string | null
  }>(rest, `library_items?id=eq.${encodeURIComponent(libraryItemId)}&select=id,presence_scope`)
  const itemRow = itemRows[0]
  if (!itemRow) refuse(`Library item not found: ${libraryItemId}`)

  const item: ContentHealthItemSnapshot = {
    id: itemRow.id,
  ...
  const fileRows = await restGet<{
    ...
    extraction_status: string | null; extraction_truncated: boolean | null; needs_review: boolean | null
  }>(
    rest,
    `library_item_files?library_item_id=eq.${encodeURIComponent(libraryItemId)}&select=id,library_item_id,extraction_status,extraction_truncated,needs_review`,
  )
  const files: ContentHealthFileSnapshot[] = fileRows.map((f) => ({
    id: f.id,
    library_item_id: f.library_item_id,
    extraction_status: f.extraction_status ?? '',
    extraction_truncated: Boolean(f.extraction_truncated),
    needs_review: Boolean(f.needs_review),
  }))

  console.log(`[run-library-content-health-helper] item ${libraryItemId} · ${files.length} file(s) · mode=${runMode} · helper v${LIBRARY_CONTENT_HEALTH_HELPER_VERSION}`)

  // Deterministic inspection (clean item → nothing). Then stamp each draft.
  const drafts = inspectLibraryContentHealth(item, files)
  if (drafts.length === 0) {
    console.log('[run-library-content-health-helper] No content-health issues found — nothing to deposit.')
    process.exit(0)
  }
  const stamped = drafts.map((d) =>
    stampRunnerMetadata(d, { helperVersion: LIBRARY_CONTENT_HEALTH_HELPER_VERSION, dedupeKey: dedupeKeyForDraft(d), runId, runMode }),
  )

  // Dry run: preview the candidates and write NOTHING (no dedupe query, no insert).
  if (dryRun) {
    console.log(`\n[run-library-content-health-helper] DRY RUN — ${stamped.length} candidate(s), nothing written:`)
  ...
  if (plan.toInsert.length === 0) {
    console.log(`[run-library-content-health-helper] All ${stamped.length} candidate(s) already active (dedupe) — nothing new deposited.`)
    process.exit(0)
  }

  const inserted = await insertHelperOutputs(restHelperOutputClient(rest), plan.toInsert, { testOwned: !depositReal })

  console.log(`\n[run-library-content-health-helper] Deposited ${inserted.length} row(s) · skipped ${plan.skipped.length} (dedupe) · run_id=${runId} · test_owned=${!depositReal}`)
...
main().catch((e) => { console.error(`\n[run-library-content-health-helper] ERROR: ${e?.message ?? e}\n`); process.exit(1) })

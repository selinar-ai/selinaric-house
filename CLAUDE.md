...
- **Governance Kernel** (`src/lib/agents/`, Phase 42.3.0 constitution): generic, domain-agnostic seams (`AgentFinding` envelope, `Inspector` contract, in-memory registry, report builder) — **generalisation proven across TWO packs with no kernel change**:
  ...
  - Posture: ephemeral health reports + an opt-in durable store + a review/triage-only Maintenance Room + representation-only remedy plans + an append-only Tara approval surface; **no apply workers, no rollback path, no standing policies, no scheduler, no LLM — still no hands.**
  - Governance boundary: persisted findings / remedy plans / approval events are **not Memory / not evidence / not authority / not graph proposals / not helper outputs / not queued work / not apply instructions**; no DB writes to any existing House source surface; no Memory / Graph approval / Archive truth / Recall / prompt-eligibility / canonical mutation. **The kernel may inspect, report, durably record, let Tara review/triage, represent a proposed remedy, and record Tara's approval decision; it may not act.**
  - **Phase 42.3.4c** = future apply / audit / rollback threshold (the actual hand) — its own constitution-level Tier-3 gate, and **must revalidate the plan against reality again immediately before any apply** (approval-time revalidation is not sufficient).

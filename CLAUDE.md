...
## Current build state
- Latest shipped: **Phase 42.4.1 — Deterministic Graph Proposal Pack** (main / production commit `16f530c`, deployed; migration `088_agent_graph_proposals.sql` applied manually by Tara — "Success. No rows returned"). `agent_graph_proposals` (agent-side, append-only, deny-by-default RLS): the kernel's first graph-proposal helper — **deterministic, suggest-only, NO LLM**. Proposes candidate `shared_source` edges between existing **approved** `archive_graph` nodes sharing `source_item_ids`; target pinned to `archive_graph`, edge-only, `shared_source` only. The record RPC is the authority boundary — verifies both endpoints approved + same archive + canonical `from<to`, computes the **live** shared-source intersection (null/blank excluded) and stores it (not caller input), DB-verifiable plain dedupe key, provenance guards (`run_id` / sha256 `input_hash` / non-blank `rationale`), skips existing edges + active dupes. Content immutable post-insert; `reviewed_by` server-derived `tara`; triage-only (`open`/`acknowledged`/`dismissed`) via `/agents`; CLI runner (`agent-graph-propose.ts`, explicit `--archive-name`, hard caps). Flags: `is_graph_proposal=true`, `not_graph_truth=true`, `is_llm_generated=false`, `not_memory=true`, `prompt_eligible=false`, `authority_changed=false`. Smoke (test-owned, archive `velvet`): 46 proposals generated, dedupe rerun 0/46, sample DB-verified, triage + cleanup clean, **no graph-truth/Memory/prompt mutation, no LLM**. **Proposal ≠ graph truth ≠ Memory; crowns nothing.** **42.4.2 (LLM proposal layer, behind these proven validators) = future Tier-3 gate, NOT started; no LLM authorised.** Prior on main: 42.3.4c — The Hand / governed apply + rollback (`5b8d6ab`, migration `087` applied); 42.3.4b — Tara Approval Authority Event (`471ed2b`, migration `086` applied); 42.3.4a — Remedy Representation (`d65aebd`, migration `085` applied); 42.3.3b — Maintenance Room Review Surface (`da1320d`, migration `084` applied); 42.3.3a — Durable Findings Store + Persistence Ingestion (`1334b17`, migration `083` applied); 42.3.2 — `archive_graph` read-only pack (`392572c`); 42.3.1 — Governance Kernel skeleton + Library read-only Health Report (`482e803`); 41.17.2 — Deterministic Helper Roster Pack (`56b812f`); migrations `081`/`082` live. See the `docs/phase-*` closure records for full phase history.
